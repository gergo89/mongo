...
namespace mongo {
	...
	public:
		...
		FakeScope(FakeScriptEngine* engine){
			jvmCreated = false;
		}
		...
		virtual void reset(){
			cout << "function called:  reset\n";
		}

		virtual void init(const BSONObj* data){
			cout << "function called: init\n";
		}
		...
		virtual void localConnect(const char* dbName) {
			cout << "function called: ft\n";
		};

		virtual void externalSetup() {
			cout << "function called: externalSetup\n";
		}

		virtual void setLocalDB(const string& localDBName) { _localDBName = localDBName; }
		virtual BSONObj getObject(const char* field)
		{
			cout << "function called: getObject\n";
			BSONObj b;
			return b;
		}

		virtual string getString(const char* field) {
			cout << "function called: getString\n";
			return 0;
		}

		virtual bool getBoolean(const char* field)
		{
			cout << "function called: getBoolean\n";
			return false;
		}
		virtual double getNumber(const char* field)
		{
			cout << "function called: getNumber\n";
			return 0;
		}
		...
		virtual void setElement(const char* field, const BSONElement& e){
			cout << "function called: setElement\n";
		}

		virtual void setNumber(const char* field, double val){
			cout << "function called: setNumber\n";
		}

		virtual void setString(const char* field, const StringData& val){
			cout << "function called: setString\n";
		}

		virtual void setObject(const char* field, const BSONObj& obj, bool readOnly = true) {
			cout << "function called: setObject\n";
		}

		virtual void setBoolean(const char* field, bool val) {
			cout << "function called: setBoolean\n";
		}
		virtual void setFunction(const char* field, const char* code){
			cout << "function called: setFUnction\n";
		}

		virtual int type(const char* field){
			cout << "function called: type\n";
			return 1;
		}

		virtual void append(BSONObjBuilder& builder, const char* fieldName, const char* scopeName){
			cout << "function called: append\n";
		}

		virtual void rename(const char* from, const char* to){
			cout << "function called: rename\n";
		}

		virtual string getError(){
			cout << "function called: getError\n";
			return std::string();
		}

		virtual bool hasOutOfMemoryException(){
			cout << "function called: hasOutOfMemoryException\n";
			return false;
		}

		virtual void installBenchRun(){
			cout << "function called: installBenchRun\n";
		}

		bool isKillPending()
		{
			cout << "function called: isKullunkPending\n";
			return false;
		}

		virtual void gc() {
			cout << "function called: gc\n";
		}
		...
		virtual ScriptingFunction createFunction(const char* raw){
			return 0;
		}
		...
		*/
		...
		virtual int invoke(ScriptingFunction func, const BSONObj* args, const BSONObj* recv,
			int timeoutMs = 0, bool ignoreReturn = false, bool readOnlyArgs = false,
			bool readOnlyRecv = false){
				cout << "function called: the longer invoke funcito from the original source \n";

				return 0;
		}

		virtual void injectNative(const char* field, NativeFunction func, void* data = 0) {
			cout << "function called: injectNative\n";
		}

		virtual bool exec(const StringData& code, const string& name, bool printResult,
			bool reportError, bool assertOnError, int timeoutMs = 0){
				cout << "function called: exec\n";
				return true;
		}

		virtual bool execFile(const string& filename, bool printResult, bool reportError,
			int timeoutMs = 0){
				cout << "function called: execFile\n";
				return true;
		}

		virtual ScriptingFunction _createFunction(const char* code,
			ScriptingFunction functionNumber = 0){
				cout << "function called: _createFunction\n";
				return 0;
		}

		virtual bool isKillPending() const{
			cout << "function called: isKillPending\n";
			return false;
		}
	};
	...
	public:
		...
		virtual Scope* createScope() { return  new FakeScope(this); }// new FakeScope(this); }
		virtual void runTest() {}
		bool utf8Ok() const { return true; }
	};
